...
set.seed(3)
X <- matrix(2, 21, 21)
X[11, 11] <- 1 #in to out fire
#X[21,] <- 1 #right to left fire 
#X[,21] <- 1 #top to bottom fire 

# big fires
# X <- forest.fire(X, 0.1, 0.2, TRUE)
X <- forest.fire(X, 0.3, 0.4, TRUE)
# medium fires
# X <- forest.fire(X, 0.07, 0.2, TRUE)
# X <- forest.fire(X, 0.2, 0.4, TRUE)
# small fires
# X <- forest.fire(X, 0.05, 0.2, TRUE)
# X <- forest.fire(X, 0.07, 0.4, TRUE)
